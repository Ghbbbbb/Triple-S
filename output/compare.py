...
def compare_positions(txt_data, json_data, env, threshold=0.05):
    correct_count = 0
    sum_error = 0
    ...
    for i, (txt_item, json_item) in enumerate(zip(txt_data, json_data)):
        ...
        if env == 'env1':
            ...
            gripper_pos_error = calculate_error(txt_item['goal_pos']['gripper_pos'], json_item['gripper_pos'])
            block1_goal_error = calculate_error(txt_item['goal_pos']['block1_goal'], json_item['block1_goal'])
            block2_goal_error = calculate_error(txt_item['goal_pos']['block2_goal'], json_item['block2_goal'])
            block3_goal_error = calculate_error(txt_item['goal_pos']['block3_goal'], json_item['block3_goal'])
            block4_goal_error = calculate_error(txt_item['goal_pos']['block4_goal'], json_item['block4_goal'])
        ...
        if env == 'env1':
            ...
            total_entry_error = gripper_pos_error + block1_goal_error + block2_goal_error + block3_goal_error + block4_goal_error + gripper_state_error
        ...
        if total_entry_error < threshold:
            ...
            if env == 'env1':
                ...
                error_details.update({
                    'block1_goal_error': block1_goal_error,
                    'block2_goal_error': block2_goal_error,
                    'block3_goal_error': block3_goal_error,
                    'block4_goal_error': block4_goal_error
                })
...
accuracy, incorrect_lines, detailed_errors, mean_error = compare_positions(txt_data, json_data, args.env)
